const quizWordSets = {
    ...
    "Quiz 1.1": {
        "尊ぶ": ["とうとぶ", "To respect", "To honor", "伝統文化を尊ぶ。", "dentō bunka o tōtobu.", "To respect traditional culture."],
    ...
    "Quiz 4.2": {
        ...
        "権力": ["けんりょく", "Authority", "Authority", "権力を乱用する。", "kenryoku o ranyō suru.", "Abuse authority."],
...
function createQuizzes() {
    for (const quizSetName in quizWordSets) {
        ...
        words.forEach(word => {
            ...
            const correctEnglishMeaning = meanings[1] || meanings[2] || meanings[0]; // Primary English meaning
            const secondaryEnglishMeaning = meanings[2] || meanings[0]; // Secondary English meaning

            const options = [correctEnglishMeaning]; // Start with the correct English option
            let wrongOptions = [];

            // 1. Add secondary English meaning of the current word as wrong option if it's different
            if (secondaryEnglishMeaning && secondaryEnglishMeaning !== correctEnglishMeaning) {
                wrongOptions.push(secondaryEnglishMeaning);
            }

            // 2. Fill wrong options with primary English meanings from *other* words in the same quiz set
            ...
            while (wrongOptions.length < 3 && otherWordIndex < otherWords.length) {
                ...
                const otherPrimaryEnglishMeaning = otherWordMeanings[1] || otherWordMeanings[2] || otherWordMeanings[0]; // Get primary English meaning from another word
            ...
            // 3. Pad with "None of the above" if still less than 4 options
        ...
        // Kanji Quiz - No changes needed for Kanji quiz options
        ...
        words.forEach(word => {
            ...
            kanjiQuizQuestions.push({
                ...
                options: [word, words[Math.floor(Math.random() * words.length)], words[Math.floor(Math.random() * words.length)], words[Math.floor(Math.random() * words.length)]], // Option 0 is correct
                correct: 0,
...
function showFeedback(isCorrect, explanationText, exampleSentence, sentenceTranslation, sentenceMeaning) {
    ...
    if (sentenceMeaning) { // Add sentence meaning (English translation)
        ...
        meaningDiv.style.animation = 'fadeInSlideUp 0.3s forwards 1.0s'; /* Faster animation duration */
        if (nightMode) {
            meaningDiv.classList.add('night-mode-sentence'); // You can style night mode sentence meaning if needed
        }
    }
...
function nextQuestion() {
    ...
    if (currentQuestionIndex < quizzes[currentQuiz].length) {
        ...
        alert(`Quiz Finished! Correct Answers: ${correctAnswers}, Wrong Answers: ${wrongAnswers}`);
        restartQuiz();
    }
}
